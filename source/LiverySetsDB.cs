...
namespace LiveryGUIMod {
    ...
    public static class LiverySetsDB {
        ...
        public static bool IsPilotLiveryAssignedToDescendantPart(int mechInstanceId, string partKey, string pilotIdOverride = null) {
            if (mechInstanceId < 0 || string.IsNullOrEmpty(partKey))
                return false;
            ...
            foreach (var kv in pilotSet.Assignments) {
                ...
                if (IsDescendantPartKey(partKey, kv.Key))
                    return true;
            }
